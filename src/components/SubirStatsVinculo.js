...
let SubidaStatsVínculo = (props) => {
  ...
  let pushStats = async () => {
    try {
      ...
      player.amistad = props.amistadConseguida;
     
      history.push({
        pathname: "/confirmar-vinculo",
        state: { player: player, dificultad: props.dificultad, vinculo: props.vinculos },
      });
    } catch (err) {
      console.error(err);
    }
...
export default withAuth(SubidaStatsVínculo);
